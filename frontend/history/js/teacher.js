...
function extractImageTokens(value) {
  ...
  const images = [];
  let index = 0;
  const displayText = text.replace(imagePattern, (_token, alt, url) => {
    index += 1;
    images.push({ id: index, alt: alt || '图片', url });
    return `[[图片${index}]]`;
  }).replace(/[ \t]+\n/g, '\n').replace(/\n{3,}/g, '\n\n').trim();
...
function composeImageText(plainText, images) {
  const sourceText = String(plainText || '').trim();
  const imageMap = new Map((images || []).filter((item) => item && item.url).map((item) => [String(item.id), item]));
  let usedIds = new Set();
  let output = sourceText.replace(/\[\[图片(\d+)\]\]/g, (_token, id) => {
    const image = imageMap.get(String(id));
    if (!image) return '';
    usedIds.add(String(id));
    return `![${image.alt || '图片'}](${image.url})`;
  });

  const trailingImages = (images || [])
    .filter((item) => item && item.url && !usedIds.has(String(item.id)))
    .map((item) => `![${item.alt || '图片'}](${item.url})`);

  if (trailingImages.length > 0) {
    output = [output, trailingImages.join('\n')].filter(Boolean).join('\n');
  }
  return output;
}
...
function setImageAwareData(inputEl, value, previewEl) {
  if (!inputEl) return;
  const { displayText, images } = extractImageTokens(value);
  inputEl.value = displayText;
  inputEl.dataset.images = JSON.stringify(images);
  inputEl.dataset.nextImageSeq = String(images.reduce((max, item) => Math.max(max, Number(item.id) || 0), 0));
...
function addEditorImage(inputEl, image) {
  ...
  const nextId = Math.max(0, Number(inputEl.dataset.nextImageSeq || data.images.length)) + 1;
  const imageItem = { id: nextId, alt: image.alt || '图片', url: image.url };
  data.images.push(imageItem);
  insertTextAtCursor(inputEl, `[[图片${nextId}]]`);
  inputEl.dataset.images = JSON.stringify(data.images);
  inputEl.dataset.nextImageSeq = String(nextId);
...
function removeEditorImage(button, index) {
  ...
  if (removed?.id) {
    const tokenPattern = new RegExp(`\\[\\[图片${removed.id}\\]\\]`, 'g');
    inputEl.value = inputEl.value.replace(tokenPattern, '').replace(/[ \t]+\n/g, '\n').replace(/\n{3,}/g, '\n\n').trim();
  }
  inputEl.dataset.images = JSON.stringify(data.images);
  renderImagePreview(inputEl, previewEl);
}
...
function addOption(value = '') {
  ...
  optionDiv.innerHTML = `
    <div class="option-edit-row">
      ...
      <input type="text" class="option-input image-aware-input" value="${escapeHtml(parsed.displayText)}" placeholder="请输入选项文字，例如 A. 数据" style="flex: 1; padding: 8px; border: 1px solid #ddd; border-radius: 6px;">
  ...
  input.dataset.nextImageSeq = String(parsed.images.reduce((max, item) => Math.max(max, Number(item.id) || 0), 0));
