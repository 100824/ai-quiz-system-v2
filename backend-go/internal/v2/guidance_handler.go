...
func guidanceSystemPrompt(phase string) string {
	...
	if phase == "evaluation" {
		return strings.Join([]string{
			...
			"【输入数据】",
			"- 本节课学习目标",
			"- 学生的预测分数与第一次小测正式成绩（满分5分，对应5道选择题）",
			"- 第一次小测作答详情（每道题的题干、学生选项、正确答案、是否答对及解析）",
			"- 若学生进行了重测：重测成绩与重测逐题作答详情；重测不覆盖第一次正式成绩",
			"- 学生本课第一至第三部分的其他作答内容",
			"- 学生过往学习表现（每课的学习目标、预测分数、实际分数；若该字段为空，说明学生无前期数据）",
			"【任务要求】",
			"1. 为学生生成本节课的简短学习评价与反思引导；",
			"2. 逐题分析错题：指出学生错在哪道题、正确解法是什么、错误可能反映的知识点漏洞，并将错题与本节课学习目标对应；",
			"3. 若学生进行了重测：比较两次成绩和逐题表现，指出已经改正、仍未掌握或重测中新出现的问题，并引导学生思考发生变化的原因；明确第一次成绩仍是正式成绩；",
			"4. 若有前期数据：结合学生长期表现趋势，分析其预测准确性与学习进步情况；",
			"5. 若无前期数据：仅依据本节课表现生成评价，不臆造学生过往表现；",
			"6. 以提问方式引导学生反思，而非只直接给出结论；",
			"7. 语言简短、鼓励为主，符合小学生的理解水平。",
			common,
		}, "\n")
	}
	return strings.Join([]string{
